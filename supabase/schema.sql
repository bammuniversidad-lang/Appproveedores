...
-- novedades) -- sin este índice compuesto, esa agregación tiene que leer
-- la tabla completa de entradas cada vez que crece con nuevas cargas.
...
--
-- OPTIMIZADO (a raíz de "canceling statement due to statement timeout" en
-- Nivel de servicio): esta función se llama una vez POR LÍNEA en
-- v_ns_proveedores, así que cualquier cosa lenta aquí se multiplica por
-- miles de filas en cada consulta. Dos cambios:
--   1. "not in (select ...)" se cambió por "not exists (...)" -- se
--      comporta igual pero Postgres lo planifica de forma más confiable
--      como anti-join contra el índice de dias_festivos_colombia, en vez
--      de re-materializar la subconsulta.
--   2. Se agregó un límite de seguridad: si el rango entre las dos fechas
--      supera ~10 años, es casi seguro una fecha mal cargada (por ejemplo
--      una fecha en 1900) y NO vale la pena recorrer día por día miles de
--      veces solo para esa fila -- se devuelve null para que se revise
--      manualmente, en vez de arrastrar la consulta completa a un timeout.
-- =====================================================================
